...
function RenderComboPlant({plant, addToCart}){
  ...
  onMouseOut={ cancelHover}>
   ...
   <Card >
    ...
    <Link to ={`/comboplants/${plant.id}`} style={{ textDecoration: 'none', color: 'black' }} >    
...
const show = props.combo.map((plant) => {
    
    return(
       <RenderComboPlant plant = {plant} onClick ={ props.onClick} key={plant.id}  addToCart={props.addToCart}/>
          );
});
...
return(
    ...
    <Breadcrumb style={{backgroundColor: '#F8F8F8'}}>
                    ...
                    <BreadcrumbItem active>Combo Plants</BreadcrumbItem>
                </Breadcrumb>
